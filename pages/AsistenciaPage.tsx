            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
                <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg" onClick={e=>e.stopPropagation()}>
                    ...
                    <footer className="p-4 flex justify-end gap-2 bg-slate-50 dark:bg-slate-800/50 rounded-b-2xl">
                        ...
                        <Button onClick={handleGenerate} disabled={isGenerating} icon={isGenerating ? () => <Loader2 className="animate-spin" /> : Download} aria-label="Generar Reporte">{isGenerating ? 'Generando...' : 'Generar'}</Button>
                    </footer>
                </motion.div>
            </motion.div>
        )}
        ...
        {alert && (
            <div className="space-y-4">
                ...
                <div className="pt-4 flex gap-2">
                    <Button variant="secondary" aria-label="Contactar / Notificar">Contactar / Notificar</Button>
                    <Button aria-label="Registrar Seguimiento">Registrar Seguimiento</Button>
                </div>
            </div>
        )}
    </Drawer>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition:{ delay: 0.3 } }} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
